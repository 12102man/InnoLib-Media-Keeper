...
@db_session
def test1():
    logging.warning("Starting Test 1")
    try:
        flush_db()
        ...
        Media(mediaID=3, name="The Mythical Man-month (Second edition), 1995",
              ...
              publisher="Addison-Wesley Longman Publishing Co., Inc.", cost=1000, fine=100, availability=0,
              bestseller=0)
        ...
        User(telegramID=324148065, name="Sergey Afonso", alias="@sergei", phone="30001", address="Via Margutta, 3",
             faculty=1)
        User(telegramID=239514818, name="Nadia Teixeira", alias="@nadia", phone="30002", address="Via Sacra, 13",
             faculty=0)
        ...
        logging.warning("Test 1 is Successful")
    except AssertionError:
        logging.warning("Test 1 is Failed")


@db_session
def test2():
    logging.warning("Starting Test 2")
    try:
        test1()
        ...
        commit()
        logging.warning("Test 2 is Successful")
    except AssertionError:
        logging.warning("Test 2 is Failed")
...
def test5():
    ...
    try:
        ...
        assert (User.get(telegramID=client2_telegram_ID) is None)
...
def test7():
    ...
    try:
        ...
        time.sleep(2)
        # Users' books
        first_user_books, second_user_books = log_select()
        ...
        assert (first_user_books[0].mediaID.startswith("1"))
        assert (first_user_books[1].mediaID.startswith("2"))
        assert (first_user_books[2].mediaID.startswith("4"))
...
@db_session
def log_select():
    return list(Log.select(lambda c: c.libID == client1_telegram_ID)), list(Log.select(lambda c: c.libID == client2_telegram_ID))
"""
...
def test7():
    ...
    try:
        ...
        with db_session:
            # Users' books
            first_user_books = list(Log.select(lambda c: c.libID == client1_telegram_ID))
            second_user_books = list(Log.select(lambda c: c.libID == client2_telegram_ID))
